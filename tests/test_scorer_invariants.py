...
from confluence_retriever.ranking import rank_results, score_result
...
class TestBasicScorerInvariants:
    def test_title_phrase_match_outranks_excerpt_only(self):
        ...
        t = score_result(title_page, ["authentication"], None)
        e = score_result(excerpt_page, ["authentication"], None)
        assert t > e
    ...
    def test_multi_token_all_match_beats_single_token(self):
        ...
        assert score_result(full, q, None) > score_result(partial, q, None)
    ...
    def test_space_match_adds_exactly_1(self):
        ...
        b = score_result(base, ["authentication"], "MT")
        s = score_result(same_space, ["authentication"], "MT")
        assert s - b == 1
    ...
    def test_title_hit_bonus_applied_in_enhanced_mode(self):
        ...
        q = ["auth"]
        assert score_result(no_hit, q, None, enhanced=False) == \
               score_result(with_hit, q, None, enhanced=False)
        assert score_result(with_hit, q, None, enhanced=True) > \
               score_result(no_hit, q, None, enhanced=True)

    def test_proximity_bonus_for_two_distinct_tokens(self):
        """Two distinct tokens close together trigger the proximity bonus."""
        assert _proximity_bonus("deploy authentication here", ["deploy", "authentication"]) == 2
    ...
    def test_proximity_no_bonus_when_tokens_far_apart(self):
        ...
        assert _proximity_bonus(far, ["deploy", "authentication"]) == 0
...
class TestProximityPhaseD:
    def test_proximity_does_not_trigger_on_single_repeated_token(self):
        ...
        assert _proximity_bonus("auth and auth again", ["auth"]) == 0
    ...
    def test_proximity_returns_zero_for_single_token_list(self):
        ...
        assert _proximity_bonus("authentication is required for auth", ["auth"]) == 0
...
class TestRecencyPhaseI:
    def test_additive_recency_does_not_boost_zero_relevance_page(self):
        ...
        z = score_result(zero_relevance_fresh, ["authentication"], None,
                               enhanced=True, halflife_days=30)
        r = score_result(medium_relevance_old, ["authentication"], None,
                               enhanced=True, halflife_days=30)
    ...
    def test_huge_halflife_matches_no_halflife_score(self):
        ...
        q = ["authentication"]
        no_recency = score_result(r, q, None, enhanced=True, halflife_days=None)
        huge_halflife = score_result(r, q, None, enhanced=True, halflife_days=9999999)
    ...
    def test_legacy_scorer_recovers_non_enhanced_score(self):
        ...
        legacy = score_result(r, q, None, enhanced=False)
        ultra_rollback = score_result(r, q, None, enhanced=False, halflife_days=None)
